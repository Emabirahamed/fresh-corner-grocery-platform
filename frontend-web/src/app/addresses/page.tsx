...
export default function AddressesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setShowForm(false)
        setForm({ label: 'home', recipient_name: '', phone: '', address_line1: '', address_line2: '', floor_number: '', apartment_number: '', landmark: '', area: '', thana: '', district: 'Dhaka', latitude: '', longitude: '', is_default: false })
        fetchAddresses()
        alert('✅ ' + data.message)
      } else { alert('❌ ' + data.message) }
    } catch { alert('ঠিকানা সংরক্ষণ করতে সমস্যা হয়েছে') }
  ...
  const handleDelete = async (addressId: number) => {
    ...
    } catch { alert('মুছতে সমস্যা হয়েছে') }
  }
  ...
  const getLabelInfo = (label: string) => LABELS.find(l => l.value === label) || LABELS[2]
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={form.recipient_name} onChange={(e) => setForm({...form, recipient_name: e.target.value})} placeholder="পূর্ণ নাম" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">ফোন নম্বর *</label>
                  <div className="flex">
                    <span className="px-3 py-2 bg-gray-100 border border-r-0 rounded-l-lg text-gray-600">+880</span>
                    <input type="tel" required value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} placeholder="1XXXXXXXXX" className="flex-1 px-3 py-2 border rounded-r-lg focus:ring-2 focus:ring-green-500" />
                  </div>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">ঠিকানা *</label>
                <input type="text" required value={form.address_line1} onChange={(e) => setForm({...form, address_line1: e.target.value})} placeholder="বাড়ি/রোড নম্বর, এলাকা" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 mb-2" />
                <input type="text" value={form.address_line2} onChange={(e) => setForm({...form, address_line2: e.target.value})} placeholder="অতিরিক্ত তথ্য (ঐচ্ছিক)" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">তলা</label>
                  <input type="text" value={form.floor_number} onChange={(e) => setForm({...form, floor_number: e.target.value})} placeholder="যেমন: ৩য়" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">অ্যাপার্টমেন্ট</label>
                  <input type="text" value={form.apartment_number} onChange={(e) => setForm({...form, apartment_number: e.target.value})} placeholder="যেমন: A-301" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>
              </div>
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">এলাকা</label>
                  <input type="text" value={form.area} onChange={(e) => setForm({...form, area: e.target.value})} placeholder="এলাকার নাম" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">থানা</label>
                  <input type="text" value={form.thana} onChange={(e) => setForm({...form, thana: e.target.value})} placeholder="থানা" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">জেলা</label>
                  <select value={form.district} onChange={(e) => setForm({...form, district: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
              ...
              <div>
                ...
                <input type="text" value={form.landmark} onChange={(e) => setForm({...form, landmark: e.target.value})} placeholder="কাছের পরিচিত স্থান" className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
              </div>
              <div className="bg-blue-50 rounded-lg p-4">
                <label className="block text-sm font-medium mb-2">📍 Google Maps কোঅর্ডিনেট (ঐচ্ছিক)</label>
              ...
              <button type="submit" disabled={saving} className="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 disabled:bg-gray-400 font-semibold">
        ...
        {addresses.length === 0 && !showForm ? (
          <div className="bg-white rounded-lg p-12 text-center">
            ...
            <button onClick={() => setShowForm(true)} className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700">+ ঠিকানা যোগ করুন</button>
          </div>
        ) : (
                ...
                <div key={address.id} className={'bg-white rounded-lg p-5 shadow-sm border-2 ' + (address.is_default ? 'border-green-400' : 'border-transparent')}>
                  <div className="flex justify-between items-start">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {address.is_default && <span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full">ডিফল্ট</span>}
                        </div>
                    ...
                    <div className="flex flex-col gap-2 ml-4">
                      {!address.is_default && (
                        <button onClick={() => handleSetDefault(address.id)} className="text-xs text-green-600 hover:text-green-700 border border-green-300 px-2 py-1 rounded">ডিফল্ট করুন</button>
                      )}
                      <button onClick={() => handleDelete(address.id)} className="text-xs text-red-600 hover:text-red-700 border border-red-300 px-2 py-1 rounded">মুছুন</button>
                    </div>
                  </div>
                </div>
              )
